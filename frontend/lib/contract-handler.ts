...
export async function getInstance(
  ...
  const address = pubkey.toAddress(null, btc.Address.PayToWitnessPublicKeyHash);
  const xOnlyPub =
    pubkey.toBuffer().length > 32
      ? pubkey.toBuffer().slice(1, 33)
      : pubkey.toBuffer();
...
export async function createBuyTx(
  ...
  const runeScriptSize = toHex(
    BigInt(`${runeId}${tokenPurchaseAmountLeb128}${runeOutpoint}`.length / 2)
  );
  ...
  for (let i = 2; i < prevTx.output.length; i++) {
    txOutToBufferWriter(prevTx.output[i], prevTxOutputs);
  }
  ...
  const witnesses = [
    ...
    prevTxOutputCount,
    Buffer.concat([Buffer.from("22", "hex"), scriptSellOrderP2TR.toBuffer()]),
  ...
  // Sometimes it fails (not sure why)
  ...
  const success = interpreter.verify(
    new btc.Script(""),
